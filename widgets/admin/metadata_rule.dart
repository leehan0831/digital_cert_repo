...
class MetadataRulesConfig {
  static final List<MetadataRule> rules = [
    ...
    MetadataRule(
      ...
      description: 'Must be "X.509" or "Self-signed".',
    ...
    MetadataRule(
      ...
      validate: (value) => value != null && value.toString().length >= 64,
    ),
    MetadataRule(
      fieldName: 'country',
      description: 'Country code must be 2 uppercase letters (ISO 3166).',
      validate: (value) =>
          value != null && RegExp(r'^[A-Z]{2}$').hasMatch(value.toString()),
    ),
  ];

  /// Utility method to validate a metadata map against all rules
